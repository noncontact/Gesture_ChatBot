...
def prepro_like_morphlized(data):
    print("prepro_like_morphlized")
    # 형태소 분석 모듈 객체를
    # 생성합니다.
    print("prepro_like_morphlized1")
    morph_analyzer = Okt()
    # 형태소 토크나이즈 결과 문장을 받을
    #  리스트를 생성합니다.
    print("prepro_like_morphlized2")
    result_data = list()
    ...
    for seq in tqdm(data):
        # 띄어쓰기 처리
        print("spacing")
        #spacing_seq = spacing(seq)
        print("spacing end")
        # Okt.pos 함수를 통해 품사 번호를 붙여 토큰화
        string_analyzed = morph_analyzer.pos(seq, norm=True, stem=True)
        analyzed = []
        # 품사 중 조사와 접속사 제거
        for words in string_analyzed:
            if words[1] != 'Josa':
                if words[1] != 'Conjunction':
                    analyzed.append(words[0])
        # 객체를 받고 다시 공백 문자를 기준으로 하여 문자열로 재구성 해줍니다.
        morphlized_seq = " ".join(analyzed)
        result_data.append(str(morphlized_seq))
    print("prepro_like_morphlized end")
    return result_data
...
def do_spacing(data):
    ...
    #  리스트를 생성합니다.
    result_data = list()
    # 데이터에 있는 매 문장에 대해 띄어쓰기를
    # 할 수 있도록 반복문을 선언합니다.
    for seq in tqdm(data):
        # 띄어쓰기 처리
        spacing_seq = spacing(seq)
        result_data.append(spacing_seq)
